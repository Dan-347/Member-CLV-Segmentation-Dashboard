...
- Cluster 1: Inactive Shoppers <br>
Customers in this cluster tend to shop infrequently and have been inactive for a long time. Their basket size is quite low, their purchase frequency is low, and the length of time since their last purchase is very long, which can reduce their long-term value potential. This group has the potential to fall into the churn category, so these customers need to be targeted for reactivation campaigns.
- Cluster 2: Exploratory Buyers <br> 
Customers in this cluster are still in the exploration stage of the product or service. They have a low purchase frequency, relatively small transaction values, and recency that indicates they have recently shopped. Their behavioral patterns are not yet consistent, but the loyalty of customers in this cluster can be increased. With the right campaign strategy, customers in this cluster can be guided to become more active and loyal customers in the future.
- Cluster 3: Top-Tier Loyalists <br>
This group consists of the most valuable customers, with high purchase frequency, recent transactions, large basket sizes, and very high CLV values. These customers are major contributors to the business, requiring personalized, exclusive, retention-focused marketing strategies to maintain their satisfaction and encourage long-term loyalty.
- Cluster 4: Regular Members <br>
Customers in this cluster have fairly consistent shopping patterns with regular purchase frequencies every 1–4 months. Transaction values have not yet reached the target but are relatively moderate, and the predicted CLV is also moderate. The stability of their shopping behavior makes this cluster a segment worth retaining. Light retention campaigns are suitable for maintaining customer engagement with the business.
- Cluster 5: High-Value Sleepers <br>
...
- Sales Contribution vs Transaction Contribution <br>
This scatter plot visualization is designed to provide an overview of how much each segment contributes to total sales and the overall number of transactions. This display helps the marketing team identify the segments that have the most influence on the business and understand whether the segment's contribution comes from the number of 
customers or intensive purchasing behavior.
- Recency of Last Transaction, Frequency of Transactions, dan Members by Monetary Value Range <br>
This visualization consists of three column charts, each showing the distribution of customers based on recency, frequency, and monetary value. These three graphs provide an overview of how customers are distributed across various behavior categories, ranging from transaction intensity to purchase value.
- Customer Lifetime Value by Segment <br>
This information displays the total CLV value of each segment in the form of a bar chart. With this visualization, the team can identify the most potential segments in contributing to future business, both in terms of loyalty and transaction value.
- Member Details <br>
The customer detail table contains more specific information about the identity, characteristics, and behavior of each customer, such as customer ID, customer name, segment name, gender, age, last transaction date, RFM value, total purchases, and estimated CLV value. This visualization is useful when the marketing team wants to explore or conduct in-depth analysis of individual customers.

## Conclusion
This study successfully combined probabilistic and segmentation models to understand customer behavior and value more comprehensively. The BG/NBD and Gamma-Gamma models showed fairly accurate CLV prediction performance based on quantitative and visual evaluations, and these results were used as the basis for the segmentation process. Through the K-Means algorithm with LRFM and CLV variables, six optimal clusters were obtained, each with different characteristics. All of these findings were then integrated into an interactive dashboard capable of presenting a summary of segmentation and customer behavior patterns. This dashboard has proven to help both the marketing team and the data analyst team in understanding customers more comprehensively, as well as supporting the decision-making process, particularly in determining which customer segments need to be retained or whose potential needs to be increased.
